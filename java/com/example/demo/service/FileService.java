...
public class FileService {
	 ...
	 * @param detailFileId
	 */
	public void fileDelete(Long fileDetailId) {
		Optional<FileDetailVO> result = fileDetailMapper.detailFileExist(fileDetailId);
		// result가 있으면
		if(result.isPresent()) { 
			if(result.get().getFilePath() != null) {
				//File_Path값이 있으면 실물 파일과 데이터 row를 삭제한다.
